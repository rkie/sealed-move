...
public class FinishedTest {
    ...
    public void before() {
    	...
    	Game game = gameDao.findByOwner(userDao.findOne("tony")).get(0);
	...
	public void testGetGameThatIsFinished() throws Exception {
    	String expected = messageSource.getMessage("game.finished.header", null, Locale.UK);
	...
	public void testGetGameThatIsFinishedAsPlayer() throws Exception {
    	String expected = messageSource.getMessage("game.finished.header", null, Locale.UK);
		mockMvc.perform(get(url).with(csrf()))
		.andExpect(status().isOk())
		.andExpect(content().string(containsString(expected)));
	...
	public void testGetGameButNotAllowed() throws Exception {
    	String expected = messageSource.getMessage("game.no.access", null, Locale.UK);
		mockMvc.perform(get(url).with(csrf()))
